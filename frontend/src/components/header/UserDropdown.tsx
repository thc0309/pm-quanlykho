...
function roleLabel(user: SessionUser) {
  if (user.kind === "master_admin") return "Quản trị hệ thống";
  if (user.kind === "warehouse_admin") return "Quản trị kho";
...
export default function UserDropdown({
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="mr-3 flex h-11 w-11 items-center justify-center overflow-hidden rounded-full bg-brand-50 text-sm font-semibold text-brand-700 dark:bg-brand-500/[0.12] dark:text-brand-300">
          {userInitials(user.fullName)}
        </span>

        <span className="block max-w-36 truncate mr-1 font-medium text-theme-sm">
          {user.fullName}
        </span>
        <svg
      ...
      >
        <div>
          ...
          <span className="mt-2 inline-flex rounded-full bg-gray-100 px-2 py-1 text-theme-xs font-medium text-gray-600 dark:bg-white/[0.06] dark:text-gray-300">
            {roleLabel(user)}
          </span>
        </div>
        ...
        <DropdownItem
          ...
          className="mt-4 flex items-center gap-3 rounded-lg px-3 py-2 font-medium text-gray-700 group text-theme-sm hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300"
        >
          <svg
