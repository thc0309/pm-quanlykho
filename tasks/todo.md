# Todo: Hoàn thiện form, user metadata và phân quyền v4

Status: đang thực hiện v4.1 — đã được xác nhận build toàn bộ

## Phase 1 — Rule nền và nhãn bắt buộc

- [x] [T29 Chuẩn hóa label bắt buộc cho form lõi](task-detail/task-29.md).
- [x] [T30 Chuẩn hóa label bắt buộc cho form chứng từ](task-detail/task-30.md).
- [x] Checkpoint A reviewed.

## Phase 2 — User metadata và avatar

- [x] [T31 Migration và API user metadata](task-detail/task-31.md).
- [x] [T32 Avatar upload và resize](task-detail/task-32.md).
- [x] [T33 UI user metadata](task-detail/task-33.md).
- [x] Checkpoint B reviewed.

## Phase 3 — Granular permission model

- [x] [T34 Permission catalog và migration reset](task-detail/task-34.md).
- [x] [T35 Backend enforce quyền chi tiết cho admin và metadata](task-detail/task-35.md).
- [x] [T36 Backend enforce quyền chi tiết cho chứng từ, stock, báo cáo và in](task-detail/task-36.md).
- [x] [T37 Role permission matrix UI](task-detail/task-37.md).
- [x] [T38 Permission-based navigation/action UI](task-detail/task-38.md).
- [x] Checkpoint C reviewed.

## Phase 4 — Metadata API và client

- [x] [T39 Update/status cho danh mục và đơn vị](task-detail/task-39.md).
- [x] [T40 Update/status cho vị trí kho](task-detail/task-40.md).
- [x] [T41 Update/status cho sản phẩm](task-detail/task-41.md).
- [x] [T42 Update/delete an toàn cho role](task-detail/task-42.md).
- [x] Checkpoint D reviewed.

## Phase 5 — Metadata UI actions

- [ ] [T43 UI danh mục và đơn vị](task-detail/task-43.md).
- [ ] [T44 UI vị trí và sản phẩm](task-detail/task-44.md).
- [ ] [T45 UI đối tác và role](task-detail/task-45.md).
- [ ] Checkpoint E reviewed.

## Phase 6 — Multi-line document forms

- [ ] [T46 Đơn mua nhiều dòng](task-detail/task-46.md).
- [ ] [T47 Báo giá và đơn bán nhiều dòng](task-detail/task-47.md).
- [ ] [T48 Phiếu nhập nhiều dòng với tracking theo từng dòng](task-detail/task-48.md).
- [ ] [T49 Phiếu xuất nhiều dòng](task-detail/task-49.md).
- [ ] [T50 Trả hàng nhiều dòng](task-detail/task-50.md).
- [ ] [T51 Chuyển kho và kiểm kê nhiều dòng](task-detail/task-51.md).
- [ ] Checkpoint F reviewed.

## Phase 7 — E2E và review

- [ ] [T52 Browser E2E cho rule mới](task-detail/task-52.md).
- [ ] [T53 Review và cleanup trước merge](task-detail/task-53.md).
- [ ] Checkpoint G reviewed.
